# Roche pRed Layers 4
# 
# This is the API for layer 4 of the Roche pRed project.  This swagger page describes the integrationUser APIs. These are typically used to find and retrieve study, sample and processed (signal) data and metadata for a given query.  Before carrying out any API calls you will need an API token. API tokens can be obtained under your profile within the Genestack software. Further instructions can be found [here](https://odm-user-guide.readthedocs.io/en/latest/doc-odm-user-guide/getting-a-genestack-api-token.html).  To try out calls in this swagger page:  1.  Click the 'Authorize' button below to enter your API token 2.  Scroll to the 'Parameters' section for the method you wish to try out and click the 'Try it out' button 3.  Enter parameter values that you wish to try 4.  Scroll to the bottom of the Parameters section and click the 'Execute' bar that appears   The server response will be in the section that follows.
# 
# API version: v0.1
# 


BatchOfIds <- R6::R6Class(
  'BatchOfIds',
  public = list(
    `_raw_json` = NULL,

    `firstIds` = NULL,
    `firstType` = NULL,
    `limit` = NULL,
    `offset` = NULL,
    initialize = function(`firstIds`, `firstType`, `limit`, `offset`){
      if (!missing(`firstIds`)) {
        stopifnot(is.list(`firstIds`), length(`firstIds`) != 0)
        lapply(`firstIds`, function(x) stopifnot(is.character(x)))
        self$`firstIds` <- `firstIds`
      }
      if (!missing(`firstType`)) {
        stopifnot(is.character(`firstType`), length(`firstType`) == 1)
        self$`firstType` <- `firstType`
      }
      if (!missing(`limit`)) {
        stopifnot(is.numeric(`limit`), length(`limit`) == 1)
        self$`limit` <- `limit`
      }
      if (!missing(`offset`)) {
        stopifnot(is.numeric(`offset`), length(`offset`) == 1)
        self$`offset` <- `offset`
      }
    },
    toJSON = function() {
      if (!is.null(self$`_raw_json`)) {
        return(jsonlite::fromJSON(self$`_raw_json`))
      }

      BatchOfIdsObject <- list()
      if (!is.null(self$`firstIds`)) {
        BatchOfIdsObject[['firstIds']] <- self$`firstIds`
      }
      if (!is.null(self$`firstType`)) {
        BatchOfIdsObject[['firstType']] <- self$`firstType`
      }
      if (!is.null(self$`limit`)) {
        BatchOfIdsObject[['limit']] <- self$`limit`
      }
      if (!is.null(self$`offset`)) {
        BatchOfIdsObject[['offset']] <- self$`offset`
      }

      BatchOfIdsObject
    },
    fromJSON = function(BatchOfIdsJson) {
      self$`_raw_json` <- BatchOfIdsJson

      BatchOfIdsObject <- jsonlite::fromJSON(BatchOfIdsJson)
      if (!is.null(BatchOfIdsObject$`firstIds`)) {
        self$`firstIds` <- BatchOfIdsObject$`firstIds`
      }
      if (!is.null(BatchOfIdsObject$`firstType`)) {
        self$`firstType` <- BatchOfIdsObject$`firstType`
      }
      if (!is.null(BatchOfIdsObject$`limit`)) {
        self$`limit` <- BatchOfIdsObject$`limit`
      }
      if (!is.null(BatchOfIdsObject$`offset`)) {
        self$`offset` <- BatchOfIdsObject$`offset`
      }
    },
    toJSONString = function() {
       if (!is.null(self$`_raw_json`)) {
         return(self$`_raw_json`)
       }

       sprintf(
        '{
           "firstIds": [%s],
           "firstType": %s,
           "limit": %d,
           "offset": %d
        }',
        lapply(self$`firstIds`, function(x) paste(paste0('"', x, '"'), sep=",")),
                if (is.null(self$`firstType`)) {
                    'null'
                } else {
                        sprintf('"%s"', self$`firstType`)
                }
            ,
                if (is.null(self$`limit`)) {
                    'null'
                } else {
                        self$`limit`
                }
            ,
                if (is.null(self$`offset`)) {
                    'null'
                } else {
                        self$`offset`
                }
            
      )
    },
    fromJSONString = function(BatchOfIdsJson) {
      self$`_raw_json` <- BatchOfIdsJson

      BatchOfIdsObject <- jsonlite::fromJSON(BatchOfIdsJson)
      self$`firstIds` <- BatchOfIdsObject$`firstIds`
      self$`firstType` <- BatchOfIdsObject$`firstType`
      self$`limit` <- BatchOfIdsObject$`limit`
      self$`offset` <- BatchOfIdsObject$`offset`
    }
  )
)
